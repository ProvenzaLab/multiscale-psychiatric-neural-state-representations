...
% load("StimP(12-24).mat");
load("FinalStims(check).mat")
...
for p = 1:numel(patients)
    ...
    for i = 1:numel(dateFields)
        ...
        dateStruct = patientStruct.(thisDate);
        sessionFields = fieldnames(dateStruct);
        for m = 1:numel(sessionFields)
            sessName = sessionFields{m};
            sessStruct = dateStruct.(sessName);
            
            % Iterate through hemispheres
            hemiFields = fieldnames(sessStruct);
            if isfield(sessStruct, 'BehavStartUnix')
                val = sessStruct.BehavStartUnix;
            else
                % skip or continue loop
                disp('Field BehavStartUnix not found, skipping...');
                continue;
            end
            BehavStartUnix = sessStruct.BehavStartUnix;

            for j = 1:numel(hemiFields)-1
                hemiName = hemiFields{j};
                if strcmp(hemiName, 'BehavStartUnix')
                    continue;
                end

                T = sessStruct.(hemiName);
                ...
                if ismember(patID, ["x004", "x005", "x007"])
                   ...
                   Tadd.BehavStartUnix = repmat(BehavStartUnix, height(Tadd), 1);
                   Tadd.sessName = repmat(string(sessName), height(Tadd), 1);
                   T_all = [T_all; Tadd];
                   ...
                   T_all = [T_all; Tadd];
                    ...
                    T_string.BehavStartUnix = repmat(BehavStartUnix, height(T_string), 1);
                    T_string.sessName = repmat(string(sessName), height(T_string), 1);
