...
for i = length(restingDirs):-1:1
    ...
    for j = 1:length(matFiles)
        ...
        for hem_idx = 1:n
            ...
            % --- Combined Data Table as JSON ---
            if isfield(lfp_data(hem_idx), 'combinedDataTable')
                json_file = fullfile(out_path, [name_ suffix '.json']);
                if ~exist(json_file, 'file')
                    writetable(lfp_data(hem_idx).combinedDataTable, json_file, 'FileType', 'text');
                else
                    fprintf('Skipping existing file: %s\n', json_file);
                end
            end
...
for i = 2:length(patients)
    patient = patients{i};
    root = fullfile('Z:\aDBS-49155\preprocessed-new', patient);
